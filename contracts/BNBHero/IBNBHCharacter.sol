...
interface IBNBHCharacter is IERC721Enumerable {
    ...
    function fight(address player, uint256 _attackingHero, uint enemyType, uint256 seed) external returns(uint256);
    ...
    function getTownsOfPlayer(address account) external view returns (HeroLibrary.Town[4] memory);
    function expediteHero(uint256 _heroId) external;
    function unlockLevel(uint256 _heroId) external returns(uint256);
    function resumeStaminaTimeStamp(uint256 _heroId, uint256 timeStamp) external;
    function calcHpForFreeHeroes(uint256 _heroId, uint256 timestamp) external view returns(uint256);
}