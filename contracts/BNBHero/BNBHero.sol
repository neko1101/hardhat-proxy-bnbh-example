...
contract BNBHero is Initializable, AccessControlUpgradeable, IERC721ReceiverUpgradeable {
    ...
    uint256 public numTokensToSend;
    mapping (uint256 => uint256) rewardsInCurLvl;
    ...
    function initialize (IBEP20 _bnbhToken, IBNBHCharacter _bnbhCharacter, IBNBHPool _bnbhPool, IPriceOracle _priceOracle, IRandoms _randoms) public initializer {
    ...
    function setNumTokensToSend(uint256 amount) public onlyOwner {
        numTokensToSend = amount;
    }     
    // function heros() public view returns (HeroLibrary.Hero[] memory) {
    //     return characters.heroes();
    // }
    function getHero(uint256 _heroId, bool calcTown) public view returns (HeroLibrary.Hero memory) {        
        return characters.getHero(_heroId, calcTown);
    }

    function getHeroesByOwner(address account, bool calcTown) public view returns (HeroLibrary.Hero[] memory) {
        uint256 balance = characters.balanceOf(account);
        HeroLibrary.Hero[] memory heroes = new HeroLibrary.Hero[](balance);
        for(uint256 i = 0; i < balance; i ++) {
            heroes[i] = getHero(characters.tokenOfOwnerByIndex(account, i), calcTown);
        }
        return heroes;
    }
    ...
    function getTownLevel(address account, uint8 townType) public view returns (uint256) {        
        return characters.getTownLevel(account, townType);
    }
    function _createHero(address payer, address fighter) internal returns(uint256) {
        require(address(priceOracle) != address(0), "Price Oracle was not set yet");
        uint256 characterPrice = priceOracle.getCharacterPrice();
        require(bnbhToken.balanceOf(payer) >= characterPrice, "Insufficient BNBH balance");
    ...
    function payForOperation(address payer, uint256 amount) internal {
        bnbhToken.transferFrom(payer, address(this), amount);            
        if (numTokensToSend <= bnbhToken.balanceOf(address(this))) {
            uint256 swapTokenAmount = numTokensToSend.mul(dividePercent).div(100);
            bnbhToken.transfer(address(bnbhToken), swapTokenAmount);
            bnbhToken.transfer(burnAddress, numTokensToSend.sub(swapTokenAmount));
        }
    }

    function moveHeroToBag(uint256 heroId) public {
        require(characters.ownerOf(heroId) == msg.sender, "You are not owner of this hero");
        require(heroIDsInBag[msg.sender].contains(heroId) == false, "The hero was already moved");
        require(heroIDsInBag[msg.sender].length() < maxHeroCount, "Your slots are full");
    ...
    function takeHeroFromBag(uint256 heroId) public {
        require(heroIDsInBag[msg.sender].contains(heroId) == true, "The hero does not exist in this bag");
        characters.safeTransferFrom(address(this), msg.sender, heroId);
        characters.resumeStaminaTimeStamp(heroId, block.timestamp - listings[heroId]);
        heroIDsInBag[msg.sender].remove(heroId);
        delete listings[heroId];
        emit TakeHeroFromBag(msg.sender, heroId);
    }

    function getHeroesInBag(address account) public view returns (HeroLibrary.Hero[] memory) {
        HeroLibrary.Hero[] memory heroes = new HeroLibrary.Hero[](heroIDsInBag[account].length());
        for (uint256 index = 0; index < heroIDsInBag[account].length(); index ++) {
            uint256 heroId = heroIDsInBag[account].at(index);
            HeroLibrary.Hero memory hero = characters.getHero(heroId, false);
            hero.hp = characters.calcHpForFreeHeroes(heroId, block.timestamp - listings[heroId]);      
            heroes[index] = hero;      
        }
        return heroes;
    }
    ...
    function unLockLevel(uint256 _heroId) public {
        ...
        if (rewardsInCurLvl[_heroId] < 0.01 ether) {
            rewardsInCurLvl[_heroId] = 0.01 ether;
        }
        price = rewardsInCurLvl[_heroId].mul(price).mul(20).div(100).div(10 ** 18);        
        require(bnbhToken.balanceOf(msg.sender) >= price, "Insufficient BNBH balance");        
        uint256 level = characters.unlockLevel(_heroId);
        rewardsInCurLvl[_heroId] = 0;
        payForOperation(msg.sender, price);
        emit UnlockedLevel(msg.sender, _heroId, level);
    }

    function getPriceToUnlockLevel(uint256 _heroId) public view returns (uint256) {
        uint256 price = priceOracle.getTokenPrice();
        if (rewardsInCurLvl[_heroId] < 0.01 ether) {
            return (price).mul(10 ** 16).mul(25).div(100).div(10 ** 18);      
        }
        return rewardsInCurLvl[_heroId].mul(price).mul(25).div(100).div(10 ** 18);      
    }

    function unpackFightData(uint256 data) internal pure returns(uint16, uint24, uint128) {
        uint16 hpLoss = uint16(data & 0xFFFF);
        uint24 xpGained = uint24((data >> 16) & 0xFFFFFF);
        uint128 bnbRewards = uint128((data >> 40) & 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF);
        return (hpLoss, xpGained, bnbRewards);
    }
    function fight(uint256 _attackingHero, uint enemyType) public {    
        require(bannedList[msg.sender] == false && isContract(msg.sender) == false);
        (uint256 fightData)= characters.fight(msg.sender, _attackingHero, enemyType, randoms.getRandomSeed(msg.sender));
        (uint16 hpLoss, uint24 xpGained, uint128 bnbRewards) = unpackFightData(fightData);
        ...
        rewardsInCurLvl[_attackingHero] = rewardsInCurLvl[_attackingHero].add(bnbRewards);
        emit Fight(msg.sender, _attackingHero, enemyType, bnbRewards, xpGained, hpLoss);
    }

    function expediteHero(uint256 _heroId) public {
        uint256 cost = priceOracle.getExpeditePrice();
        require(bnbhToken.balanceOf(msg.sender) >= cost, "Insufficient BNBH balance");
    ...
    function upgradeTown(uint8 townType) public {
        ...
        uint256 priceForUpdate = priceOracle.getTownUpgradePrice(townType, level + 1); 
        require(bnbhToken.balanceOf(msg.sender) >= priceForUpdate, "Insufficient BNBH balance");
    ...
    function claimRewards() public {
        ...
        require(block.timestamp > unLockTime[msg.sender], "You can not claim at this time");
